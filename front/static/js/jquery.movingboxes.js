	...
	$.movingBoxes = function(el, options){
		...
		base.update = function(callback, flag){
			...
			if (o.wrap && base.$panels.length > 1) {
                ...
                base.$el.prepend( base.$panels.filter(':nth-last-child(2)').clone().addClass('cloned') );
		...
		base.change = function(curPanel, callback, flag){
			...
			if (o.wrap) {
				if (curPanel > base.totalPanels) {
                    ...
                    if (o.wrap && base.$panels.length > 1) {
                        base.$el.children('.clonur').remove();
                        base.$el.prepend( base.$panels.filter(':nth-child(' + parseInt(curPanel - 1) + ')').clone().addClass('clonur').addClass('cloned') );
                    }
                    ...
                    base.$el.children('.croutos').remove();
                    base.$el.prepend( base.$panels.filter(':nth-last-child(' + parseInt(o.startPanel + 1) + ')').clone().addClass('croutos').addClass('cloned') );
				} else if (curPanel === 0) {
					wrapped = false;
