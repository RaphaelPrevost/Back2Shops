...
def get_users_urlpatterns():
    urlpatterns = staticfiles_urlpatterns()

    urlpatterns += patterns('',
        url(r'^webservice', include(settings.get_site_prefix()+'users_webservice.urls')),
    )
    return urlpatterns
...
if current_site.name == 'users':
    urlpatterns = get_users_urlpatterns()
else:
    # Uncomment the next two lines to enable the admin:
    from django.contrib import admin
    admin.autodiscover()

    urlpatterns = get_sales_urlpatterns()
